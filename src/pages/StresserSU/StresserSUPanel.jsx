...
function StresserSUPanel() {
  ...
  const handleCloseErrorModal = () => {
    ...
    const createPreset = (id, buttonId) => {
      ...
      function isValidURL(url) {
        const urlRegex = /^(?:(?:https?|ftp):\/\/)?(?:(?:[a-z0-9](?:[a-z0-9-]{0,61}[a-z0-9])?\.)+[a-z]{2,})(?::\d{1,5})?(?:\/[\w.-]*)*(?:\?[\w%&=]*)?(?:#[\w-]*)?$/i;
        return urlRegex.test(url);
      }
      const sendAttack = ()=>{
        if (layer) {
            if (!targetURL || !time || !cuncurrentValue || !originOption || !methodOption || !captchaCode ) {
                ...
                if (!isValidURL(targetURL)) {
                    setErrorMsg('Please enter Valid URL fields before Start Attacking...');
                    setShowErrorModal(true);
                }else{
                    ...
                    if (currentCaptcha === captchaCode) {
                        ...
                        console.log('Sending Attack with:', targetURL, time, captchaCode,originOption);
                        setOnGoingEmpty(true);
                        setTargetURL('');
                        setCaptchaCode('');
            ...
            if (!targetIP || !port || !time || !cuncurrentValue || !originOption || !methodOption || !captchaCode ) {
                setErrorMsg('Please enter all fields before Start Attacking...');
                setShowErrorModal(true);
            }
            else{
                if (!isValidIP(targetIP) || !isValidPort(port)) {
                    ...
                    if (currentCaptcha === captchaCode) {
                        ...
                        setAttacks([...attacks, newAttack]);
                        console.log('Sending Attack with:', targetIP,port, time,methodOption, captchaCode,originOption);
                        setPort('');
                        setTargetIP('');
                        setCaptchaCode('');
                        let reset = document.getElementById('c');
  ...
  return (
    <>
            {showErrorModal && (
  <div className="modal" id="running_modal-err" style={{ display: 'block',zIndex:10240 }}>
    <div className="modal-dialog modal-dialog-scrollable modal-md modal-dialog-centered">
     ...
     <div className="row my-5">
      <div className="col-xl-12">
        <div className="row">
          <div className={`col-xl-6 btn-group mb-3 ${activeTab === 0 ? 'active' : ''}`}>
            <a
              onClick={() => {handleTabClick(0);sevenLayer()}}
              href="#SevenLayer"
            ...
            >
              ...
              <span className="fs--2">Send HTTPS GET/POST DDoS attacks on a web server.</span>
            </a>
          </div>
          <div className={`col-xl-6 btn-group mb-3 ${activeTab === 1 ? 'active' : ''}`}>
            <a
              onClick={() => {handleTabClick(1);fourLayer()}}
            ...
            >
              ...
              <h6 className="text-base fs-0">Layer 3/4 (Network/Transport)</h6>
              <span className="fs--2">Send UDP/TCP DDoS attacks on an IPv4 target.</span>
            </a>
          </div>
        </div>
      ...
      <div className="row align-items-center">
        ...
        <div className="col-auto">
          ...
          >
            ...
            {/* Create Preset Modal */}
            <div className="modal fade" id="createpreset-7" tabIndex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div className="modal-dialog modal-lg" role="document">
          <div className="modal-content">
            <div className="modal-header">
              <h5 className="modal-title" id="exampleModalLabel">Load Preset</h5>
              <button className="btn-close" type="button" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div className="modal-body">
              <div className="form-group">
                <label htmlFor="preset-name-7">Preset Name</label>
                <input type="text" className="form-control" id="preset-name-7" name="preset-name-7" maxLength="" value="" placeholder="Ex: example.com BROWSER V2" />
              </div>
              <p>Fill out the hub form with all required information to continue creating your preset.</p>
            </div>
            <div className="modal-footer">
              <button className="btn btn-primary btn-sm" id="create-preset-7" onClick={() => createPreset(7, 'create-preset-7')} type="submit">Create</button>
              <button className="btn btn-secondary btn-sm" type="button" data-bs-dismiss="modal">Cancel</button>
            </div>
          </div>
        </div>
            </div>
            {/* Schedule Modal */}
            <div className="modal fade" id="scheduleForm-7" tabIndex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div className="modal-dialog" role="document">
          <div className="modal-content">
            {/* Modal header */}
            <div className="modal-header">
              <h5 className="modal-title" id="exampleModalLabel">Schedule Attack</h5>
              <button className="btn-close" type="button" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            {/* Modal body */}
            <div className="modal-body">
              <div className="form-group">
                ...
                <input type="date" className="form-control" id="start-date" name="start-date" maxLength="" value="" placeholder="" />
              </div>
              <div className="form-group">
                <label htmlFor="start-time">Start Time</label>
                <input type="time" className="form-control" id="start-time" name="start-time" maxLength="" value="" placeholder="" />
              </div>
            </div>
            {/* Modal footer */}
            <div className="modal-footer">
              <button className="btn btn-secondary btn-sm" type="button" data-bs-dismiss="modal">Cancel</button>
              <button className="btn btn-primary btn-sm" id="schedulebtn-7" onClick={() => scheduleAttack(7, 'schedulebtn-7')} type="submit">Create</button>
            </div>
          </div>
        </div>
            </div>
            {/* Load Preset Modal */}
            <div className="modal fade" id="loadpreset-7" tabIndex="-1" role="dialog" aria-labelledby="exampleModalLabel" style={{ display: 'none' }} aria-hidden="true">
        <div className="modal-dialog modal-lg" role="document">
          <div className="modal-content">
            <div className="modal-header">
              <h5 className="modal-title" id="exampleModalLabel">Load Preset for Hub Layer 7</h5>
              <button className="btn-close" type="button" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div className="modal-body">
              <div className="form-group">
                <label htmlFor="preset-key-7">Preset Key</label>
                <input type="text" className="form-control" id="preset-key-7" name="preset-key-7" maxLength="" value="" placeholder="XXX-XXX-XXX" />
              </div>
              <button className="btn btn-primary btn-sm" id="load-preset-7" onClick={() => importPreset(7, 'load-preset-7')} type="submit">Import</button>
              <hr />
              <h5 className="mt-3 mb-3">Imported presets</h5>
              <div className="table-responsive-lg" id="loaded-preset-7">
                <div className="text-center">
                  <div className="alert alert-primary" role="alert">No data available.</div>
                </div>
              </div>
            </div>
            <div className="modal-footer">
      ...
      {layer ? (
        <tr>
          <td>
            <div className="form-group">
              <label htmlFor="host-7" className="text-white" style={{ marginBottom: 10 }}>
                Target URL
              </label>
              <input
                type="text"
        ...
        <tr>
          <td>
            <div className="form-group">
              <label htmlFor="host-4" className="text-white" style={{ marginBottom: 10 }}>
                Target IP
              </label>
              <input
                type="text"
          ...
          <td>
            <div className="form-group">
              <label htmlFor="port-4" className="text-white" style={{ marginBottom: 10 }}>
                Port (0 = random)
              </label>
              <input
                type="text"
      ...
      />
            ...
            <div className="accordion">
              <div className="card">
                ...
                >
                  ...
                  >
                    <svg class="svg-inline--fa fa-caret-right fa-w-6 accordion-icon mr-3" data-fa-transform="shrink-2" aria-hidden="true" focusable="false" data-prefix="fas" data-icon="caret-right" role="img" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 192 512" data-fa-i2svg="" style={{transformOrigin: '0.1875em 0.5em'}}><g transform="translate(96 256)"><g transform="translate(0, 0)  scale(0.875, 0.875)  rotate(0 0 0)"><path fill="currentColor" d="M0 384.662V127.338c0-17.818 21.543-26.741 34.142-14.142l128.662 128.662c7.81 7.81 7.81 20.474 0 28.284L34.142 398.804C21.543 411.404 0 402.48 0 384.662z" transform="translate(-96 -256)"></path></g></g></svg>
                    <span className="font-weight-medium text-sans-serif text-900">
                      &nbsp; Advanced parameters
                    </span>
                  </button>
                </div>
                <div className="collapse" id="collapse-7" aria-labelledby="optionsHub">
        ...
        <tr>
          <td colSpan="3">
            ...
            <select className="custom-select mb-3" id="method-7" name="method" value={methodOption}
          onChange={handleMethodChange}>
      <optgroup style={{ color: '#ffffff' }} label="SSL/TLS (for HTTPS://)">
        <option style={{ color: '#ffe000' }} value="HTTPSOUND" disabled>
          [PREMIUM] HTTPS SOUND (HTTP / HTTP2) [CloudFlare]
        </option>
        <option style={{ color: '#ffe000' }} value="REQUEST-CUSTOM" disabled>
          [PREMIUM] HTTPS REQUEST-CUSTOM (HTTP / HTTP2) [EXPERIENCED USERS ONLY!]
        </option>
        <option style={{ color: '#ffe000' }} value="HTTPSCRYPTO" disabled>
          [PREMIUM] HTTPS-CRYPTO v2 (HTTPS2)
        </option>
        <option style={{ color: '#ffe000' }} value="HTTPSOUNDRATE" disabled>
          [PREMIUM] HTTPS SOUND-RATE (HTTP / HTTP2) [CloudFlare]
        </option>
      </optgroup>
      <optgroup style={{ color: '#ffffff' }} label="SPECIAL METHODS">
        <option style={{ color: '#ffe000' }} value="Tor" disabled>
          [PREMIUM] TOR (.onion)
        </option>
        <option style={{ color: '#ffe000' }} value="HTTPSCUSTOM" disabled>
          [ENTERPRISE] HTTPS-CUSTOM (Real Browsers)
        </option>
      </optgroup>
      <optgroup style={{ color: '#ffffff' }} label="SOCKET METHODS">
        <option style={{ color: '#ffe000' }} value="WEBSOCKET-CUSTOM" disabled>
          [PREMIUM] WEBSOCKET-CUSTOM (WS / WSS)
        </option>
        <option style={{ color: '#ffe000' }} value="SPAMMERV2" disabled>
          [PREMIUM] SPAMMER v2 (HTTPS/1.1)
        </option>
        <option style={{ color: '#ffe000' }} value="SOCKETV4" disabled>
          [PREMIUM] SOCKET V4 (HTTP/1.1)
        </option>
      </optgroup>
      <optgroup style={{ color: '#ffffff' }} label="Free L7 Methods">
        <option style={{ color: '#ffffff' }} value="HTTPSPAM">
          [FREE] HTTP-SPAM
        </option>
        <option style={{color:'#ffffff'}} value="FREENTP">FREE-NTP</option>
      </optgroup>
    </select>
          </td>
        </tr>
        ...
        <tr>
          ...
          <img src={captchaImg}  width="200" height="40" alt='captcha' style={{ marginRight:15}}  />
            <button className="btn btn-falcon-default mr-1 mb-1" type="submit" onClick={refreshCaptcha}>
              Refresh
            </button>
            <div className="form-group">
              <label htmlFor="time" className="text-white" style={{float:'left',marginBottom:13}}>Captcha Code</label>
              <input
        type="text"
          ...
          <div className="card-body">
            <div className="row">
              <div className="col-lg-8" style={{ zIndex: 2 }}>
                ...
                <p className="mt-2">Manage your ongoing and scheduled attacks.</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="card mb-3">
          <div className="card-body bg-light">
            <ul className="nav nav-pills">
              ...
              <li className="nav-item">
                ...
                >
                  ...
                  (<span id="attackongoing">{attacks.length}</span>) Ongoing
                </a>
              </li>
              <li className="nav-item">
                <a
              ...
              <li className="nav-item">
                ...
                > <i className="fa fa-power-off"></i> Stop all attacks</a>
            ...
            <div className="tab-content p-3 mt-3">
              ...
              <div className="tab-pane fade" id="scheduled" role="tabpanel" aria-labelledby="profile-tab">
                <div className="text-center">
                  <div className="alert alert-primary" role="alert">
                    No data available.
                  </div>
